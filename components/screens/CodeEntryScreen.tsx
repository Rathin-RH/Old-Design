...
interface CodeEntryScreenProps {
  onSuccess: (enteredCode: string) => void;
  isActive: boolean;
  code: string;
...
export const CodeEntryScreen: React.FC<CodeEntryScreenProps> = ({
  ...
  const handleSubmit = () => {
    ...
    if (enteredCode.length !== 6 || isLoading) return;
    ...
    >
      <div className="keypad-layout">
        ...
        <div className="modern-keypad">
          ...
          <button
            className={`num-btn submit-btn ${code.length === 6 ? 'ready' : ''}`}
            onClick={handleSubmit}
            disabled={code.length !== 6 || isLoading}
          >
            <span className="material-symbols-outlined">
              {isLoading ? 'hourglass_top' : 'check'}
            </span>
          </button>
        </div>
      </div>
